...
func (_q *SourceQuery) QueryListings() *ListingQuery {
...
func (_q *SourceQuery) WithListings(opts ...func(*ListingQuery)) *SourceQuery {
	query := (&ListingClient{config: _q.config}).Query()
...
func (_q *SourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Source, error) {
	...
	if query := _q.withListings; query != nil {
		if err := _q.loadListings(ctx, query, nodes,
			func(n *Source) { n.Edges.Listings = []*Listing{} },
			func(n *Source, e *Listing) { n.Edges.Listings = append(n.Edges.Listings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SourceQuery) loadListings(ctx context.Context, query *ListingQuery, nodes []*Source, init func(*Source), assign func(*Source, *Listing)) error {
	...
	query.Where(predicate.Listing(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(source.ListingsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.source_listings
		if fk == nil {
			return fmt.Errorf(`foreign-key "source_listings" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "source_listings" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
