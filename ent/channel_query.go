...
func (_q *ChannelQuery) QueryPriceAlerts() *PriceAlertQuery {
...
func (_q *ChannelQuery) WithPriceAlerts(opts ...func(*PriceAlertQuery)) *ChannelQuery {
	query := (&PriceAlertClient{config: _q.config}).Query()
...
func (_q *ChannelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Channel, error) {
	...
	if query := _q.withPriceAlerts; query != nil {
		if err := _q.loadPriceAlerts(ctx, query, nodes,
			func(n *Channel) { n.Edges.PriceAlerts = []*PriceAlert{} },
			func(n *Channel, e *PriceAlert) { n.Edges.PriceAlerts = append(n.Edges.PriceAlerts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ChannelQuery) loadPriceAlerts(ctx context.Context, query *PriceAlertQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *PriceAlert)) error {
	...
	query.Where(predicate.PriceAlert(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(channel.PriceAlertsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.channel_price_alerts
		if fk == nil {
			return fmt.Errorf(`foreign-key "channel_price_alerts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "channel_price_alerts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
