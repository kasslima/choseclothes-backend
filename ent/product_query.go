...
func (_q *ProductQuery) QueryListings() *ListingQuery {
...
func (_q *ProductQuery) WithListings(opts ...func(*ListingQuery)) *ProductQuery {
	query := (&ListingClient{config: _q.config}).Query()
...
func (_q *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := _q.withListings; query != nil {
		if err := _q.loadListings(ctx, query, nodes,
			func(n *Product) { n.Edges.Listings = []*Listing{} },
			func(n *Product, e *Listing) { n.Edges.Listings = append(n.Edges.Listings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProductQuery) loadListings(ctx context.Context, query *ListingQuery, nodes []*Product, init func(*Product), assign func(*Product, *Listing)) error {
	...
	query.Where(predicate.Listing(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.ListingsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.product_listings
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_listings" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_listings" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
